...
import { Breadcrumb, Flex, Box, BreadcrumbItem, BreadcrumbLink, Avatar, Button, Text, Badge, Heading, Skeleton, Divider } from '@chakra-ui/core';
        ...
        <Flex marginBottom={8}>
            ...
            {currentUser.loading ? (
                ...
                <>
                  <Heading as='h2' mb={1} fontWeight='600'>
                    {currentUser.data?.me.name} {currentUser.data?.me.surname} {currentUser.data?.me.patronymic}
                  </Heading>
                  <Flex alignItems='center' marginBottom='15px'>
                    <Text color='blue.500'>{currentUser.data?.me.email} - </Text> {currentUser.data?.me.roles.includes('ADMIN') ? (
                        <Badge marginLeft='5px' variantColor="purple" variant="solid"> Преподаватель</Badge>
                      ) : (
                        <Badge marginLeft='5px' variantColor="blue" variant="solid">Ученик</Badge>
                      )}
                  </Flex>
          ...
          <Box marginLeft='auto' marginTop='auto'>
            {currentUser.loading ? (
              ...
              <Button leftIcon={MdEdit} variantColor='blue' borderRadius='30px'>Редактировать</Button>
            )}
          </Box>
        </Flex>
        <Divider />
